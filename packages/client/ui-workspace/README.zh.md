...
分组浏览用各自的标题行命名每个 Workspace。把**分组方式**切换为单列表后只留下一份列表，因此每一行会在 Session 标题上方带上所属 Workspace 的名字；不属于任何 Workspace 的 Session 显示其目录名，两者都没有时显示本地化的 **未分组**。
...
这些限制定义搜索深度、归档界面、扁平行展示与选取载体；它们是当前包约束。
...
- **扁平行更高**：Workspace 标签行使每条扁平行至少 45px（分组单元格为 32px），因此单列表一次显示的 Session 更少。
