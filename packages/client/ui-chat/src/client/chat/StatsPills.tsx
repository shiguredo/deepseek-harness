...
export const StatsPills = memo(function StatsPills({ useChat, useProjection, t, renderSlot }: StatsPillsProps) {
  ...
  const hasStats = stats.steps > 0 || hasTokens
  ...
  // tightens the composer's bottom clearance, so it stays tied to the figures
  // that give the row its own reading.
  return (
    <div className={css.root} data-composer-stats={hasStats ? '' : undefined}>
