...
在携带设置传输（`ui-settings`）、locale registry 与 slot registry 的 Web 组合中挂载本插件；web-app bundle 会把它挂在 `ui-theme` 旁边。本插件不接受配置。

### 行为预期

「一般」设置中会增加一行 Font：一个持有 CSS font-family 列表的单行输入框，按 Enter 或失焦时提交。提交的列表会经过校验（去除首尾空白、最长 256 字符、不得包含 `;`、`{}` 或控制字符），持久化到 `ui-font-family` 设置 section，并写入文档 body 的 `--dsw-font-family` 与 `--ds-font-family-code`，因此一次选择同时覆盖界面文字与代码。清空输入框会清除持久值并移除两个 token，使文档回到主题随附的字体栈；被校验拒绝的文本会留在输入框中并标记为无效，便于修正。保存了字体族时，索引引导会内联它，因此重新加载后的首次绘制在任何 client 插件激活之前就已带上它。
...
当可选的 settings 服务被组合时，`apply` 会在 `ui-font-family` 下注册 `FontSettingsSchema`；当持久字体族已设置时，对每次 `webserver/index-inject` 收集回答一个 body 脚本。未保存字体族时不产生任何行，主题随附的字体栈保持默认。
...
- [dsh-client-ui-settings](../ui-settings/README.zh.md) — 设置 scope、schema 校验与「一般」item 席位。
...
- **进程内页面保持随附字体栈** — 当设置传输仅存在于内存（非 loopback 页面且宿主不提供持久化）时，设置行仍可编辑但不会写入，也不会应用任何变量，这与设置 scope 自身的约定一致。
